...
public class AddAProperty1 extends Frame implements ActionListener,ItemListener
{
	Modifiers m;
	char choice;
			...
			for(int i=0;i<14;i++)
			{
				...
				else
				{
					...
					else if(choice=='Y'|choice=='y')
					{
						try
						{
							String s=null,frnhd=null;
							if(sell.getState()==true)
								s="buy";
							else if(rent.getState()==true)
								s="rent";
							...
							String insert_qry="insert into "+s+" ()value(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
							PreparedStatement stmt=db.con.prepareStatement(insert_qry);
							stmt.setString(1,);
							stmt.setString(2,);
							stmt.setString(3,);
							stmt.setString(4,);
...
{
	AddAProperty1 a;
	MyWindowListener(AddAProperty1 a)
	{
		this.a=a;
	}
